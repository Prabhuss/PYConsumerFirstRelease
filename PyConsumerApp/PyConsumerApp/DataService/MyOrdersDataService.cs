...
namespace PyConsumerApp.DataService
{
    ...
    public class MyOrdersDataService : BaseService
    {
         ...
         */
        public async Task<string> generateCFtoken(Dictionary<string, string> payload, Dictionary<string,string> header)
        {
            try
            {
                Dictionary<string, string> robject = await this.Post<Dictionary<string, string>>("https://test.cashfree.com/api/v2/cftoken/order", JsonConvert.SerializeObject(payload), header);
